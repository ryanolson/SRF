...
#include "srf/coro/schedule_policy.hpp"
...
class RingBuffer
{
  ...
  public:
    ...
    {
        WriteOperation(RingBuffer<ElementT>& rb, ElementT e) :
          m_rb(rb),
          m_e(std::move(e)),
        ...
        auto await_ready() noexcept -> bool
        {
            ...
            // m_write_span->AddEvent("start_on", {{"thead.id", srf::this_thread::get_id()}});
    ...
    struct ReadOperation : ThreadLocalContext
    {
        ...
        auto await_ready() noexcept -> bool
        {
            // the lock is owned by the operation, not scoped to the await_ready function
            m_lock = std::unique_lock(m_rb.m_mutex);
            // m_read_span->AddEvent("start_on", {{"thead.id", srf::this_thread::get_id()}});
